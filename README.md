...
5. 친구의 뉴스피드 리스트를 조회하는 기능을 구현할 때, NewsfeedRepository와 NewsfeedService를 참조하도록 진행했었습니다.
   아무래도 책임 분리 측면에서 좋지 않은 코드인 것 같아 NewfeedService를 참조하는 코드를 최대한 friend에 구현하여 NewsfeedRepository만 참조하도록 변경했습니다.
6. 타입 안정성 측면이나 상태 값 관리할 때의 효율을 위해 친구 요청 승인 여부의 상태 값을 enum 타입의 FriendStatus로 처리했습니다. status를 String이 아닌 FriendStatus로 받도록 진행하였습니다.
